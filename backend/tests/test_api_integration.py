...
from backend.app.api.v2.analyses import router as analyses_router
from backend.app.core.config import Settings
from backend.app.db.repo import AnalysisRepository, PHASE_DONE, STATUS_DONE
...
def _build_test_client(tmp_path: Path, max_upload_bytes: int = 1024 * 1024, complete_immediately: bool = True) -> TestClient:
    ...
    app.include_router(analyses_router, prefix="/api/v2")
...
def test_analysis_lifecycle_success(tmp_path: Path):
    ...
    status_resp = client.get(f"/api/v2/analyses/{analysis_id}/status")
    assert status_resp.status_code == 200
    assert status_resp.json()["status"] == "done"

    dashboard = client.get(f"/api/v2/analyses/{analysis_id}/dashboard")
...
def test_list_analyses_returns_created_item(tmp_path: Path):
    ...
    listing = client.get("/api/v2/analyses?limit=20")
...
def test_delete_analysis_removes_record(tmp_path: Path):
    ...
    deleted = client.delete(f"/api/v2/analyses/{analysis_id}")
    assert deleted.status_code == 204

    missing = client.get(f"/api/v2/analyses/{analysis_id}/status")
...
def test_analysis_status_rejects_invalid_id_format(tmp_path: Path):
    client = _build_test_client(tmp_path)
    response = client.get("/api/v2/analyses/not-a-uuid/status")
...
def test_dashboard_returns_500_for_corrupted_payload(tmp_path: Path):
    ...
    response = client.get(f"/api/v2/analyses/{analysis_id}/dashboard")
