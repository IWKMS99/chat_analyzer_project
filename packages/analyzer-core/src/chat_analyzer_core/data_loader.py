...
_DEFAULT_FALLBACK_JSON_MAX_BYTES = 50 * 1024 * 1024


def _fallback_json_max_bytes() -> int:
    raw = os.getenv("CHAT_ANALYZER_FALLBACK_JSON_MAX_BYTES")
    if not raw:
        return _DEFAULT_FALLBACK_JSON_MAX_BYTES
    try:
        value = int(raw)
    except ValueError:
        return _DEFAULT_FALLBACK_JSON_MAX_BYTES
    return value if value > 0 else _DEFAULT_FALLBACK_JSON_MAX_BYTES
...
def _iter_messages(file_path: str) -> Iterator[Dict[str, Any]]:
    fallback_limit = _fallback_json_max_bytes()
    try:
