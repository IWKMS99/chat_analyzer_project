...
This document describes runtime variables for API/analyzer and web container runtime overrides.
...
| `MAX_UPLOAD_BYTES` | `104857600` (100 MB) | API upload endpoint | Maximum accepted upload size at FastAPI layer. | Must be tuned together with `NGINX_CLIENT_MAX_BODY_SIZE`; effective limit is the lower value. |
| `NGINX_CLIENT_MAX_BODY_SIZE` | `100m` | nginx front proxy | Maximum request body accepted by nginx before API. | Increase when handling bigger uploads; keep in sync with `MAX_UPLOAD_BYTES`. |
...
| `VITE_API_BASE_URL` | `/api` | Web app runtime config (with build-time fallback) | Base URL/prefix for API calls. | In prebuilt images, value is injected at container startup; keep `/api` behind nginx, use full URL for split deployments. |
...
- Copy root `.env.example` to `.env` and adjust values per environment.
- For local API run, `scripts/dev/run_api.sh` applies safe defaults for:
