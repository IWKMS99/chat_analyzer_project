...
def _available_columns(rows: DatasetRows) -> set[str]:
    out: set[str] = set()
    for row in rows:
        out.update(row.keys())
    return out
...
def _safe_chart(
    ...
    *,
    ...
    if color_field:
        required.add(color_field)
    if tooltip_fields:
        required.update(tooltip_fields)
...
def _safe_wide_bar(
    ...
    *,
    ...
    value_columns = [column for column in metrics if column in available]
    if not value_columns:
        return _fallback_chart(dataset_name, rows)
...
def _chart_temporal_response(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        ...
        rows,
        mark="bar",
        x_field="response_min",
        x_type="quantitative",
        y_field="count",
        semantic_kind="distribution",
...
def _chart_user_message_counts(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        dataset_name,
        rows,
        mark="bar",
...
def _chart_user_avg_length(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        dataset_name,
        rows,
        mark="bar",
...
def _chart_user_chains(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_wide_bar(
        dataset_name,
        rows,
        x_field="from",
        x_type="nominal",
        metrics=["avg_chain", "median_chain", "max_chain"],
        semantic_kind="categorical_breakdown",
    ) or _fallback_chart(dataset_name, rows) or _chart(
        dataset_name,
        "bar",
...
def _chart_message_lengths(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_wide_bar(
        dataset_name,
        rows,
        x_field="from",
...
def _chart_message_question_ratio(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        dataset_name,
        rows,
        mark="bar",
        ...
        x_type="nominal",
        y_field="ratio",
        tooltip_fields=["from", "ratio", "questions", "total"],
        semantic_kind="categorical_breakdown",
        chart_kind="bar",
    ) or _fallback_chart(dataset_name, rows) or _chart(
        dataset_name,
...
def _chart_nlp_keywords(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        dataset_name,
        rows,
        mark="bar",
...
def _chart_nlp_vocabulary(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        dataset_name,
        rows,
        mark="bar",
        x_field="from",
        x_type="nominal",
        y_field="lexical_diversity",
        tooltip_fields=["from", "lexical_diversity", "total_words", "unique_words"],
        semantic_kind="categorical_breakdown",
        chart_kind="bar",
    ) or _fallback_chart(dataset_name, rows) or _chart(
        dataset_name,
...
def _chart_nlp_emoji(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        dataset_name,
        rows,
        mark="bar",
...
def _chart_anomaly_anomalies(dataset_name: str, rows: DatasetRows) -> ChartDefinition:
    return _safe_chart(
        dataset_name,
        rows,
        mark="line",
...
_REGISTRY: dict[tuple[str, str], Callable[[str, DatasetRows], ChartDefinition | None]] = {
    ...
    ("user", "message_counts"): _chart_user_message_counts,
    ("user", "avg_length"): _chart_user_avg_length,
    ("user", "chains"): _chart_user_chains,
    ("user", "daily_by_user"): _chart_user_daily_by_user,
    ("message", "lengths"): _chart_message_lengths,
    ("message", "question_ratio"): _chart_message_question_ratio,
    ...
    ("anomaly", "anomalies"): _chart_anomaly_anomalies,
}
